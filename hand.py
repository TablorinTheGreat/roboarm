...
class camera_angle:
    ...
    def __init__(self,cameraNum,client):
        ...
        cv2.rectangle(self.frame, (self.last_box[0], self.last_box[1]), (self.last_box[0] + self.last_box[2], self.last_box[1] + self.last_box[3]),
                      (50, 50, 50), cv2.FILLED)
    ...
    def findHand(self,andmask,frame):
        ...
        for i in range(1, ret):
            ...
            diff=self.diff(np.array(flatblob), np.array(flathand))
    ...
    def getHandMask(self,skinMask,moveMask,frame):
        ...
        # cv2.imshow("clear",clearedBackground)
    ...
    def main(self):
        ...
        while True:
            ...
            for (x, y, w, h) in faces:
                cv2.rectangle(self.frame, (int(0.9*x), int(0.7*y)),( int((x + w)), int(1.2*(y + h))), (255, 255, 0), cv2.FILLED)
            ...
            if self.handOnScene:
                ...
                self.frameCount+=1
                cv2.putText(self.frame, str(self.frameCount), (10, 500), cv2.FONT_HERSHEY_SIMPLEX, 4, (255, 255, 255), 2, cv2.LINE_AA)
                ...
                if handMask is not None:
                    ...
                    cv2.circle(self.frame, (self.hand[self.handTitles["loc"]][0], self.hand[self.handTitles["loc"]][1]), 7, (0, 255, 255), -1)

                    # draw the hands bounding box
                    cv2.rectangle(self.frame, (self.last_box[0], self.last_box[1]), (self.last_box[0] + self.last_box[2], self.last_box[1] + self.last_box[3]), (255, 100, 0), 2)
                ...
                if curr >= self.startingThreshold and cv2.waitKey(1) & 0xFF == ord(
