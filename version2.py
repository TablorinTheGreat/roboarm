...
def findHand(andmask,frame,handm):
    ...
    for i in range(1, ret):
        ...
        l.append(simi(np.array(flatblob),np.array(flathand)))
        if handBlob is None:
            maxSim = sim
            handMask = c
            handBlob=blob
...
def clear_background(skinMask,moveMask,frame,gray,handm):
    ...
    add(flat(list(hand.values())))
    last_box=BRec
    hand=handblob
    return handMask
def updateColorValues(median,spread):
    return (median+spread).clip(0,255)
def updateColors(YCbCr_image,HSV_image,Lab_image,mask=None):
    global lower_YCbCr_values,upper_YCbCr_values,lower_HSV_values,upper_HSV_values,lower_Lab_values,upper_Lab_values,frameCount,colorUpdateInterval,lower_color_extension,upper_color_extension
...
def color_segmentation(img,mask=None,gray=None):
    ...
    updateColors(YCbCr_image,HSV_image,Lab_image,mask)
...
def sendAngle():
    ...
    return 0
roboRot(0)
weights=flat(list(weightsDict.values()))
handMask=start.copy()
...
ft=flat(list(titles.values()))
for i in range(len(info)):
    plt.subplot(8, 1, i+1)
    plt.plot(info[i], label=titleDict[ft[i]])
    plt.legend()
plt.subplot(8, 1, 8)
